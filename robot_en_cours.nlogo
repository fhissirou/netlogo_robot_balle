...
to bouge
  ask Robots[
    ...
    ifelse(any? robots-on patch-right-and-ahead 90 1)
    [
      set gauche 90
    ]
    [
      if(any? robots-on patch-left-and-ahead 90 1)
      [set  droite 90]
    ]
    ...
    [
      ...
      set robot-tas (go-to-objectif xcor ycor coord-devine-x coord-devine-y robot-tas value-propietaire)
...
let tas 0
  ask pieces with[piece-propietaire = value-propietaire][
   ;; if ((ycor <= (coord-y + 7)) and (ycor >= (coord-y - 7)))[
      if ((xcor <= (coord-x + 7)) and (xcor >= (coord-x - 7)))[
        ...
        set ycor coord-devine-y
          ...
          if(tas = 2)[ set xcor (xcor - 1)]
          if(tas = 3)[ set xcor (xcor + 1)]
...
"" ""
PENS
"robots" 1.0 0 -16777216 true "Robots" "plot count robots"
